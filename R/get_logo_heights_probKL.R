#' @title Get heights of logos in nlogomaker() using probability KL weighting of symbols as in seq2Logo.
#'
...
#' scales respectively using a probability Kullback Leibler weighting scheme
#' similar to seq2Logo.
#'
...
get_logo_heights_probKL <- function(table, epsilon = 0.01, bg = NULL,
  ...
  table <- apply(table+0.0001,2,normalize_probKL)
  bgmat <- apply(bgmat+0.0001,2,normalize_probKL)
...
normalize_probKL = function(x){return(x/sum(x[!is.na(x)]))}
