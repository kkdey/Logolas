...
#' colnames(m) = 1:12
#' m=m/8
#' get_logo_heights_ic_diff(m, alpha = 1, hist = FALSE)
#'
#' @export
...
get_logo_heights_ic_diff <- function(table, alpha = 1, epsilon = 0.01, bg = NULL, opt = 1,
  ...
  table <- apply(table+0.0001,2,normalize_ic_diff)
  bgmat <- apply(bgmat+0.0001,2,normalize_ic_diff)
...
normalize_ic_diff = function(x){return(x/sum(x[!is.na(x)]))}
