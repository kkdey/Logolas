...
#'
#' @param color_type A list containing two elements - "type" and "col". The type can
...
logo_pssm <- function(table,
  ...
  if(color_type == "per_row"){
    if(is.null(colors)){
      ...
      set.seed(color_seed)
      color_profile <- list("type" = color_type,
                            "col" = sample(colors, dim(table)[1], replace = FALSE))
      }
    }
  if(color_type == "per_symbol"){
    if(is.null(colors)){
      ...
      set.seed(color_seed)
      color_profile <- list("type" = color_type,
                            "col" = sample(col_vector, length(control$total_chars), replace=FALSE))
    }else{
      if (length(colors) < length(control$total_chars)){
        stop("For per_symbol color type, the colors vector must be as large as number of
             symbols in total_chars argument in control() : which is 50 by default ")
      }
      set.seed(color_seed)
      color_profile <- list("type" = color_type,
                            "col" = sample(colors, length(control$total_chars), replace=FALSE))
      }
  }
  if(color_type == "per_column"){
    if(is.null(colors)){
      ...
      set.seed(color_seed)
      color_profile <- list("type" = color_type,
                            "col" = sample(col_vector, dim(table)[2], replace = FALSE))
    }else{
      ...
      set.seed(color_seed)
      color_profile <- list("type" = color_type,
                            "col" = sample(colors, dim(table)[2], replace = FALSE))
      }
    }
    ...
    for (j in seq_len(npos)){
      ...
      for (i in seq_along(chars)){
        ...
        if(length(intersect(letterOrder[i], slash_inds))!=0){
          if (ht>0) letters <- addLetter_pssm(letters,letter, tofill = control$tofill, lwd = control$lwd, col, total_chars, x.pos, y.pos, ht, wt[j], scale0 = scale0, scale1=scale1, addlogos = addlogos, addlogos_text = addlogos_text)
        }else{
          if (ht>0) letters <- addLetter_pssm(letters,letter, tofill = control$tofill, lwd = control$lwd, col, total_chars, x.pos, y.pos, ht, wt[j], scale0 = scale0, scale1=scale1, addlogos = NULL, addlogos_text = NULL)
        }
  ...
  if(color_profile$type == "per_symbol"){
    for (j in seq_len(npos)){
      ...
      for (i in seq_along(chars)){
        ...
        ht <- hts[letterOrder[i]]
        if(length(intersect(letterOrder[i], slash_inds))!=0){
          if (ht>0) letters <- addLetter_pssm(letters,letter, tofill = control$tofill, lwd = control$lwd, color_profile$col, total_chars, x.pos, y.pos, ht, wt[j], scale0 = scale0, scale1=scale1, addlogos = addlogos, addlogos_text = addlogos_text)
        }else{
          if (ht>0) letters <- addLetter_pssm(letters,letter, tofill = control$tofill, lwd = control$lwd, color_profile$col, total_chars, x.pos, y.pos, ht, wt[j], scale0 = scale0, scale1=scale1, addlogos = NULL, addlogos_text = NULL)
        }
        y.pos <- y.pos + ht + start
      }
      x.pos <- x.pos + wt[j]
  ...
  if(color_profile$type == "per_column"){
    for (j in seq_len(npos)){
      ...
      for (i in seq_along(chars)){
        ...
        ht <- hts[letterOrder[i]]
        if(length(intersect(letterOrder[i], slash_inds))!=0){
          if (ht>0) letters <- addLetter_pssm(letters,letter, tofill = control$tofill, lwd = control$lwd, color_profile$col[j], total_chars, x.pos, y.pos, ht, wt[j], scale0 = scale0, scale1=scale1, addlogos = addlogos, addlogos_text = addlogos_text)
        }else{
          if (ht>0) letters <- addLetter_pssm(letters,letter, tofill = control$tofill, lwd = control$lwd, color_profile$col[j], total_chars, x.pos, y.pos, ht, wt[j], scale0 = scale0, scale1=scale1, addlogos = NULL, addlogos_text = NULL)
        }
        y.pos <- y.pos + ht + start
      }
      x.pos <- x.pos + wt[j]
