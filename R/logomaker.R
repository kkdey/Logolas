#' @title Create logo plots from aligned sequences or positional frequency (weight) matrix
#'
#' @description Takes as input a vector of character sequences (aligned to have the )
#' same length or a positional frequency or weight matrix and plots the standard logo
#' or the Enrichment Depletion (ED) Logo plots.
#'
#' @param data The input data may be a vector of character sequences - representing
#'             aligned sequences of DNA, RNA or amino acids, or a matrix/ data frame with
#'             symbols of characters or strings of characters along the rows of the matrix/data
...
#'                 based on the number of underlying seqeunces. Step automatically
#'                 skipped if the user provides a positional weight matrix.
#'
...
#'                      for which, based on \code{color_seed}, a specific coloring scheme
#'                      is chosen. The list contains two elements - \code{type} and \code{col}.
#'                      The \code{type} can be of three types - "per-row", "per-column" and "per-symbol".
#'                      The \code{col} element is a vector of colors, of same length as number of rows
#'                      in table for "per-row" (assigning a color to each string), of same length
#'                      as number of columns in table for "per-column" (assuming a color for each column),
#'                      or a distinct color for a distinct symbol in "per-symbol". For "per-symbol", the length
#'                      of the \code{color_profile$col} should be same as library size of the logos,
#'                      but if the vector of colors provided is more or less, we can downsample or upsample
#'                      the colors as required. The colors are matched with the symbols in the \code{total_chars}.
#'
#' @param colors Add description here.
#' 
#' @param color_seed A seed for choosing among multiple available coloring schemes in \code{color_profile}.
#'                   The default choice is 2030. But the user can use any seed of her choice.
#'
#' @param return_heights Boolean. If TRUE, the function returns the stack heights for the logo plot.
#'                       For standard Logo (type = "Logo"), it returns the information content.
#'                       For tyep = "EDLogo", it returns the total stack height along positive and
#'                       negative axis, as well as the breakdown of the heights along different symbols
...
#' @return Generates a standard or an EDLogo plot - depending on the \code{type} variable.
#'
#' @examples
#'
  ...
  if(type == "Logo"){
   ...
   if(is.character(data)){
        ...
        if(is.null(color_type)){
          message("color_type not provided, so switching to per_row option for color_type")
          color_type = "per_row"
        }
        if(color_type == "per_row"){
          if(is.null(colors)){
            ...
            if (length(colors) < dim(pfm_scaled)[1]){
              stop("For per_row color type, the colors vector must be as large as number of
                   rows in the matrix for PFM/PWM input, or number of distinct characters in
                   each aligned sequence for sequence data")
            }
            set.seed(color_seed)
            color_profile <- list("type" = color_type,
                                  "col" = sample(colors, dim(pfm_scaled)[1], replace = FALSE))
          }
        }
        if(color_type == "per_symbol"){
          if(is.null(colors)){
            ...
            set.seed(color_seed)
            color_profile <- list("type" = color_type,
                                  "col" = sample(col_vector, length(logo_control$total_chars), replace=FALSE))
          }else{
            if (length(colors) < length(logo_control$total_chars)){
              stop("For per_symbol color type, the colors vector must be as large as number of
                   symbols in total_chars argument in logo_control() : which is 50 by default ")
            }
            set.seed(color_seed)
            color_profile <- list("type" = color_type,
                                  "col" = sample(colors, length(logo_control$total_chars), replace=FALSE))
          }
        }
        if(color_type == "per_column"){
          if(is.null(colors)){
            ...
            set.seed(color_seed)
            color_profile <- list("type" = color_type,
                                  "col" = sample(col_vector, dim(pfm_scaled)[2], replace = FALSE))
          }else{
            if (length(colors) < dim(pfm_scaled)[2]){
              stop("For per_column color type, the colors vector must be as large as number of
                   columns in the matrix for PFM/PWM input, or number of characters in each aligned
                   sequence for sequence data")
            }
            set.seed(color_seed)
            color_profile <- list("type" = color_type,
                                  "col" = sample(colors, dim(pfm_scaled)[2], replace = FALSE))
          }
        }
   ...
   if(!is.character(data)){
     ...
     if(!is.matrix(data) & !is.data.frame(data)){
       stop("if not a character sequence, data must be either a matrix or a data frame")
     }

     if(is.null(rownames(data))){
       stop("row names of the  data matrix should be symbols to be plotted in the logo plot ")
     }
     ...
     if(min(data[!is.na(data)]) < 0){
       stop("negative values in data matrix not permitted : try logo_pssm () function for plotting position specific
            scores")
     }
     ...
     if(color_type == "per_row"){
       if(is.null(colors)){
         ...
         set.seed(color_seed)
         color_profile <- list("type" = color_type,
                               "col" = sample(col_vector, dim(data_scaled)[1], replace = FALSE))
       }else{
         ...
         set.seed(color_seed)
         color_profile <- list("type" = color_type,
                               "col" = sample(colors, dim(data_scaled)[1], replace = FALSE))
         }
       }
     if(color_type == "per_symbol"){
       if(is.null(colors)){
     ...
     if(color_type == "per_column"){
       if(is.null(colors)){
         ...
         set.seed(color_seed)
         color_profile <- list("type" = color_type,
                               "col" = sample(col_vector, dim(data_scaled)[2], replace = FALSE))
       }else{
         ...
         set.seed(color_seed)
         color_profile <- list("type" = color_type,
                               "col" = sample(colors, dim(data_scaled)[2], replace = FALSE))
         }
       }
