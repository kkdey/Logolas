...
#'
#' @description Generates total heights of the stack of logos in the positive and negative
...
#'
#' @param symm A bool input, which if TRUE, the function uses symmetric KL divergence
#' whereas if FALSE, the function uses non-symmetric KL divergence.
#'
...
get_logo_heights <- function (table,
  ...
  if(length(score) != 1){
    stop("score can be wither diff, log, log-odds, probKL, ratio, unscaled_log or wKL")
  }
  ...
  if(!ic){
    ...
    if(score == "diff"){
      ...
      if(opt == 1){
        ...
        table_mat_adj <- apply((table_mat_norm+epsilon)*log(table_mat_norm+epsilon, base=2), 2, function(x)
        {
          indices <- which(is.na(x))
          if(length(indices) == 0){
            y = x
  ...
  if(!ic){
    ...
    if(symm==TRUE){
      ...
      ic <- 0.5*(ic_computer(table_mat_norm, alpha, hist=hist, bg = bgmat) + ic_computer(bgmat, alpha, hist=hist, bg = table_mat_norm))
    }else{
      table_mat_norm[which(is.na(table))] <- NA
